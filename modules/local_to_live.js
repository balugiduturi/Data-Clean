...
var MongoSettings = {
    local: {
        ...
        projects_collection: "private_data_15052017",
...
function iterateBatch() {
    (async function () {
        ...
        var query = {
            "_id": ObjectId("5919cbd7d73c682d578b4568")
        }

        cursor = await local_dbo.collection(local_collection).find(query, exclude_objects_updated).limit(limit).batchSize(100);
        ...
        cursor.forEach(async (doc) => {
            try {
                if (doc !== null || doc !== 'undefined') {
                    ...
                    doc["_log"] ? _log = true : "";
                    doc["dates"] ? dates = true : "";

                    let formatted = formatfields(doc);
                    if (!formatted.error) {
                        res = await live_dbo.collection(live_collection).save(formatted.doc);
                    }
                    ...
                    if (_log) {
                        ...
                        update_doc_local["_log.dp_moved"] = dp_moved;
                    }
                    if (dates) {
                        delete update_doc_local["dates"];
                        update_doc_local["dates.dp_moved"] = new Date();
                    }

                    await local_dbo.collection(local_collection).update({ _id: doc._id }, { $set: update_doc_local });
                    counter++;

                    if (counter % 1000 === 0) {
                        console.log(`Moved Documents are ${counter}`);
                    }

                    if (counter >= 1) {
                        await local_dbo.close();
                        await local_connection.close();
                        await live_dbo.close();
                        await live_connection.close();
                        var endTime = Date.now();
                        var variation = endTime - startTime;
                        console.log("seconds elapsed = " + Math.floor(variation / 1000));
                        process.exit(3);
                    }
                } else {
                    console.log("All Documents are moved error");
                }
            } catch (E) {
                console.log("welcome to Error Catch ")
